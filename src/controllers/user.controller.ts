...
export async function subscribePro(req: Request, res: Response) {
  ...
  try {
    const sessionUrl = await StripeService.createProCheckoutSession(userId, userEmail);
    return res.status(200).json({ success: true, url: sessionUrl });
  } catch (err: any) {
    return res.status(500).json({ success: false, message: err.message || 'Failed to create Stripe session' });
  }
}
...
export async function getSubscriptionStatus(req: Request, res: Response) {
  ...
  try {
    ...
    return res.status(500).json({ success: false, message: err.message || 'Failed to get subscription status' });
  }
} 