...
export class StripeService {
  static async createProCheckoutSession(userId: number, userEmail: string): Promise<string> {
  ...
  static async handleWebhook(request: any, sig: string): Promise<any> {
    ...
    if (event.type === 'customer.subscription.created' || event.type === 'customer.subscription.updated' || event.type === 'customer.subscription.deleted') {
      ...
      if (userId) {
        ...
        const currentPeriodEnd = new Date(((subscription as any).current_period_end * 1000) || (subscription as any).currentPeriodEnd * 1000);
  ...
  static async getUserSubscriptionStatus(userId: number): Promise<{ tier: string; status: string; currentPeriodEnd: Date | null }> {
    const db = await dbPromise;
